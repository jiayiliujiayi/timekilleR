...
#' Fast read matrix with duplicated rownames
...
#' Fast read matrix with unique rownames
#'fast read a dataset into a matrix
#'
#' @param path_to_file the path to the dataset
#' @param delim delimiter, default (tab-delimited)
...
#' @param cores number of cores when reading the dataset. default: 29.  Why 29? Because I love the prime numbers.
#' @param method defalut = "sum"
#' @param blank rename the blank rownames as
#'
#' @return
#' @export
#' @import data.table
...
#' @examples
fread.matrix2 <-
  function(path_to_file, method = sum, delim = "auto", header = "auto", cores = 29, blank = 'novel'){
    # set path to the file as 'input' variable
    input = path_to_file
    ...
    # return matrix
    matrix.temp <- df.temp
    # rename the blank rownames
    rownames(matrix.temp)[which(rownames(matrix.temp) == '')] = blank
